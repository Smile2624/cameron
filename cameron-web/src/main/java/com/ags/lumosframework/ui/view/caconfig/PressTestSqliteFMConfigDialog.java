...
@SpringComponent
@Scope("prototype")
public class PressTestSqliteFMConfigDialog extends BaseDialog {


    TextField tfPath = new TextField("压力数据存放目录（Sqlite阀门）：");
    ...
    protected void initUIData() {
        caConfig = caConfigService.getConfigByType(AppConstant.CA_CONFIRM_PRESSTEST_SQLITE_FM);
    ...
    protected void okButtonClicked() throws Exception {
        if (Strings.isNullOrEmpty(tfPath.getValue().trim())) {
            throw new PlatformException("压力数据存放目录（Sqlite阀门）未填写！");
        } else {
            //TODO 路径校验
            if (caConfig == null) {
                caConfig = new CaConfig();
            }
            caConfig.setConfigType(AppConstant.CA_CONFIRM_PRESSTEST_SQLITE_FM);
    ...
    public void show(UI parentUI, DialogCallBack callBack) {
        ...
        showDialog(parentUI, "压力数据存放目录（Sqlite阀门）配置", "350px", null, false, true, callBack);
    }
}
